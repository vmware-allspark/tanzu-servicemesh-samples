...
# Read the name space that was piped into this script.
[ ! -t 0 ] && NAMESPACE=$( cat /dev/stdin)
...
kubectl delete -n ${NAMESPACE} -f "$SCRIPTDIR/bookinfo-SRE-optimized.yaml" > "${OUTPUT}" 2>&1
ret=$?
function cleanup() {
  rm -f "${OUTPUT}"
}
